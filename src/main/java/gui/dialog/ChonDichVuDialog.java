...
public class ChonDichVuDialog implements Initializable {
    ...
    public void setPhien(String maPhien, String phienInfo) {
        ...
        if (lblPhienInfo != null) lblPhienInfo.setText("Phiên: " + phienInfo);
    }
    ...
    private void setupTableColumns() {
        ...
        if (colTenDV != null) colTenDV.setCellValueFactory(new PropertyValueFactory<>("tendv"));
        if (colDonGia!= null) {
        ...
        if (colTonKho!= null) colTonKho.setCellValueFactory(new PropertyValueFactory<>("soluongton"));

        // Cart columns
        if (colCartTen      != null) colCartTen.setCellValueFactory(new PropertyValueFactory<>("tenDV"));
        if (colCartSL       != null) colCartSL.setCellValueFactory(new PropertyValueFactory<>("soLuong"));
        if (colCartDonGia   != null) colCartDonGia.setCellValueFactory(new PropertyValueFactory<>("donGiaFormatted"));
        if (colCartThanhTien!= null) colCartThanhTien.setCellValueFactory(new PropertyValueFactory<>("thanhTienFormatted"));
    }
    ...
    private void setupTableSelection() {
        tableDichVu.getSelectionModel().selectedItemProperty().addListener((obs, o, n) -> {
            ...
            if (lblSelectedDV != null)
                lblSelectedDV.setText(n != null ? n.getTendv() + " (" + String.format("%,.0f ₫", n.getDongia()) + ")" : "Chưa chọn");
        });
    }
    ...
    public void handleSearch() {
        ...
        tableDichVu.setItems(dichVuList.filtered(d ->
            kw.isEmpty() || d.getTendv().toLowerCase().contains(kw)));
    }

    @FXML
    public void handleIncrease() {
        if (txtSoLuong == null) return;
        try { txtSoLuong.setText(String.valueOf(Integer.parseInt(txtSoLuong.getText()) + 1)); }
        catch (NumberFormatException e) { txtSoLuong.setText("1"); }
    }
    ...
    public void handleAddToCart() {
        if (selectedDV == null) { setError("Vui lòng chọn dịch vụ"); return; }
        int qty;
        try { qty = Integer.parseInt(txtSoLuong != null ? txtSoLuong.getText() : "1"); }
    ...
    public void handleXacNhan() {
        ...
        if (maPhien == null || maPhien.isEmpty()) { setError("Chưa có thông tin phiên"); return; }
    ...
    private void updateCartTotal() {
        ...
        if (lblTongTienCart != null) lblTongTienCart.setText(String.format("%,.0f ₫", total));
    }
    ...
    public static class CartItem {
        ...
        public String getDonGiaFormatted()    { return String.format("%,.0f ₫", getDonGia()); }
        public String getThanhTienFormatted() { return String.format("%,.0f ₫", getThanhTien()); }
    }
}
